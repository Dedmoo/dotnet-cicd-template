...
- `http://SUNUCU_IP:5001` = sağlık kontrol adresi  

**Yasak:** Uzak sunucuda `http://127.0.0.1:5001` yazmak. Çalışmaz. Mutlaka sunucu IP’si.
...
Bitince hata yoksa sunucu systemd birimlerini almış demektir. Uygulama henüz boş olabilir; ilk deploy doldurur.
...
3. Deploy biter → kırmızıysa otomatik geri alma denenebilir.
...
| Health fail ama bağlandınız | `SERVICES` içinde `127.0.0.1` veya firewall portu kapalı |
